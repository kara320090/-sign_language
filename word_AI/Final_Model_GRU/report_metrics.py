...
def run_report(
        ...
        model_display_name: str = "GRU Optimized"
) -> None:
...
def main():
    ...
    print(f"=== Generating Report: {config['run_name']} ===")
    
    run_report(
        data_dir=get_word_data_dir(),
        model_path=get_model_dir() / "best_model.keras",
        output_path=get_result_dir() / "metrics_table.txt",
        seed=DEFAULT_SEED,
        feature_indices=f_indices,
        model_display_name=config["run_name"]
    )
